...
const AddressSelectorPopup = ({ isOpen, onClose, onSelect }) => {
  ...
  // Sync selectedId with the current default address from the database
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-2xl bg-white rounded-[40px] border border-gray-100 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 shadow-2xl">
        {/* Header */}
        <div className="px-10 pt-10 pb-6 flex justify-between items-center">
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-full border border-gray-100 flex items-center justify-center text-gray-400 hover:text-gray-900 cursor-pointer transition-colors"
          >
            <FiX size={24} />
          </button>
        </div>
        ...
        <div className="px-10 pb-8 min-h-[350px]">
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 max-h-[50vh] overflow-y-auto no-scrollbar pr-1">
              {addresses.map(addr => (
                <div
                  key={addr.id}
                ...
                >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      {addr.is_default && (
                        <span className="px-2 py-0.5 rounded-full border border-(--success-500) bg-(--success-50) text-(--success-500) text-[10px] font-bold uppercase">
                          Default
                        </span>
                      )}
                    </div>
              ...
              <div
                ...
                className="border-2 border-dashed border-(--success-100) bg-(--success-25) rounded-[24px] p-6 flex flex-col items-center justify-center text-center gap-3 hover:bg-(--success-50) transition-all cursor-pointer group"
              >
                <div className="w-12 h-12 rounded-full bg-(--success-500) flex items-center justify-center text-white group-hover:scale-110 transition-transform shadow-lg shadow-(--success-500)/20">
        ...
        <div className="px-10 pb-10 flex flex-col sm:flex-row justify-between items-center gap-4">
          <button
            onClick={handleAddNew}
            className="w-full sm:w-auto px-8 h-14 rounded-full border border-primary-50 bg-primary-25 text-primary-500 font-bold flex items-center justify-center gap-2 hover:bg-primary-50 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="w-full sm:w-auto px-10 h-14 rounded-full bg-(--color-primary-700) text-white font-bold flex items-center justify-center gap-3 hover:bg-(--color-primary-800) transition-all cursor-pointer disabled:opacity-50 shadow-lg shadow-(--color-primary-700)/20"
          >
