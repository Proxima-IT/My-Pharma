...
/**
 * LoginForm Component
 * Updated: Supports modal mode (isModal) and success callbacks (onSuccess).
 * Used in both the standalone login page and the global AuthModal.
 */
export default function LoginForm({ isModal = false, onSuccess = null }) {
  const { formData, setFormData, isLoading, error, handleLogin } = useLogin();
  const [showPassword, setShowPassword] = useState(false);

  const onFormSubmit = async e => {
    // Invoke the existing handleLogin logic
    await handleLogin(e);

    // In a modal context, if login is successful (token exists), trigger the callback
    if (onSuccess && localStorage.getItem('access_token')) {
      onSuccess();
    }
  };
  ...
  return (
    <div
      className={`w-full flex flex-col items-center ${isModal ? 'p-0' : ''}`}
    >
      {/* Header Section - Hidden when used inside AuthModal to avoid duplication */}
      {!isModal && (
      ...
      <form onSubmit={onFormSubmit} className="w-full space-y-6">
        ...
        <div className="flex items-center justify-between px-2">
          <label className="flex items-center gap-2 cursor-pointer group">
            <input
              ...
              onChange={e =>
                setFormData({ ...formData, keepLogin: e.target.checked })
              }
            />
        ...
        {error &&
          ...
          !error.toLowerCase().includes('password') && (
            <p className="text-center text-xs font-bold text-red-500 animate-in fade-in">
              {error}
            </p>
          )}
