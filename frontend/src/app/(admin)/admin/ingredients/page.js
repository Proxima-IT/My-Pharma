...
export default function AdminIngredientListPage() {
  ...
  }, [page, search, fetchIngredients]);

  const handleDelete = async (slug, name) => {
    if (
      confirm(`Are you sure you want to delete the generic name "${name}"?`)
    ) {
      await deleteIngredient(slug);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-100 pb-6">
        <div>
          ...
          <p className="text-[13px] text-[#6B6B5E] mt-1 font-medium">
            Manage medicine formulas and generic names for your inventory.
          </p>
        </div>
        <Link href="/admin/ingredients/new">
          <button className="bg-[#3A5A40] text-white px-8 py-3.5 rounded-none font-bold text-xs tracking-widest flex items-center gap-2 hover:bg-[#F59E0B] transition-all duration-300 cursor-pointer uppercase border border-transparent">
            <FiPlus size={18} /> Add New Generic
          </button>
        </Link>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading && ingredients.results.length === 0 ? (
                <tr>
                ...
                ingredients.results.map(item => (
                  ...
                  >
                    ...
                    <td className="px-8 py-6 border-r border-gray-100">
                      <span className="font-mono text-[11px] font-bold text-[#8A8A78] uppercase">
                        ID_{item.id}
                      </span>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/admin/ingredients/edit/${item.slug}`}>
