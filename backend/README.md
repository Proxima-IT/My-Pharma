...
- **Firebase Admin SDK** - FCM web push delivery and Google ID token verification
...
1. **Environment**
   ...
   # For FCM push / Firebase Admin (choose one credential mode):
   # FIREBASE_SERVICE_ACCOUNT_JSON='{"type":"service_account",...}'
   # or GOOGLE_APPLICATION_CREDENTIALS=/run/secrets/firebase-service-account.json
   # For Google sign-in verification on backend:
...
- **Google sign-up/sign-in:** `POST /api/auth/google/` with Firebase `id_token`; backend verifies with Firebase Admin SDK and returns JWT + user.
